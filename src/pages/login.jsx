...
const makeInitialForm = () => ({
  ...
  age: "", weight: "", height: "", gender: "MALE", fitnessGoal: "WEIGHT_LOSS",
  activityLevel: "MODERATELY_ACTIVE", dailyCalorieTarget: "2000",
    ...
    try {
      if (mode === "login") {
        if (!otpMode) {
          ...
          if (res?.otpRequired) { setOtpMode(true); setSuccess(res.message || "OTP sent to your email."); return; }
      ...
      if (mode === "register") {
        ...
        setMode("login"); setOtp(""); setForm(makeInitialForm()); return;
      }
      if (mode === "forgot") {
        await forgotPassword(form.email);
        setSuccess("Password reset email sent. Check your inbox."); return;
      }
      if (mode === "reset") {
        await resetPassword(form.resetToken, form.newPassword, form.confirmPassword);
        setSuccess("Password reset! You can sign in now.");
        setMode("login"); router.replace("/login");
      }
    } catch (err) {
...
const styles = StyleSheet.create({
  ...
  logoText: { fontSize: 28, fontWeight: "900", color: "#0a0e1a" },
